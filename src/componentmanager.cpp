...
ComponentManager::ComponentManager()
{
    ...
    for (auto i = 0; i < ENITY_LUT_GROW; ++i)
        this->m_EntityComponentMap[i].resize(NUM_COMPONENTS,
...
ComponentId ComponentManager::AqcuireComponentId(IComponent* component)
{
    ...
    this->m_ComponentLUT.resize(
        this->m_ComponentLUT.size() + COMPONENT_LUT_GROW, nullptr);
...
{
    assert((id != INVALID_COMPONENT_ID && id < this->m_ComponentLUT.size()) &&
...
void ComponentManager::UnmapEntityComponent(EntityId        entityId,
                                            ...
                                            ComponentTypeId componentTypeId)
{
    assert(this->m_EntityComponentMap[entityId.index][componentTypeId] ==
               componentId &&
